...
int ecx_EOEsetIp(ecx_contextt *context, uint16 slave, uint8 port, eoe_param_t * ipparam, int timeout)
{
   ...
   wkc = ecx_mbxreceive(context, context->slavelist[slave].EoEmbxq, slave, (ec_mbxbuft *)&MbxIn, 0);
   ec_clearmbx(&MbxOut);
   aEOEp = (ec_EOEt *)&MbxIn;
   EOEp = (ec_EOEt *)&MbxOut;  
   ...
   if (wkc > 0) /* succeeded to place mailbox in slave ? */
   {
      ...
      ec_clearmbx(&MbxIn);
      /* read slave response */
      wkc = ecx_mbxreceive(context, context->slavelist[slave].EoEmbxq, slave, (ec_mbxbuft *)&MbxIn, timeout);
...
#ifdef EC_VER1
int ec_EOEsetIp(uint16 slave, uint8 port, eoe_param_t * ipparam, int timeout)
{
   return ecx_EOEsetIp(&ecx_context, slave, port, ipparam, timeout);
}
int ec_EOEgetIp(uint16 slave, uint8 port, eoe_param_t * ipparam, int timeout)
{
   return ecx_EOEgetIp(&ecx_context, slave, port, ipparam, timeout);
}
int ec_EOEsend(uint16 slave, uint8 port, int psize, void *p, int timeout)
{
   return ecx_EOEsend(&ecx_context, slave, port, psize, p, timeout);
}
int ec_EOErecv(uint16 slave, uint8 port, int * psize, void *p, int timeout)
{
   return ecx_EOErecv(&ecx_context, slave, port, psize, p, timeout);
}
#endif
